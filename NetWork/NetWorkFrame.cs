/********************************************************************************
 *	文件名：NetWorkFrame.cs
 *	全路径：\Script\NetWork\NetWorkFrame.cs
 *	创建人：	王华
 *	创建时间：2013-11-29
 *
 *	功能说明： 网络收发逻辑管理器
...
using Google.ProtocolBuffers;  //InvalidProtocolBufferException   ||  CodedOutputStream
...
public class NetWorkFrame
{
    ...
    public static NetWorkFrame GetMe()
    {
        if (m_Impl == null)
        {
            m_Impl = new NetWorkFrame();
        }
        return m_Impl;
    }
    private static NetWorkFrame m_Impl = null;
    ...
    private NetWorkFrame()
    {
        ...
        m_PacketFactoryManager.Init();//初识化包工厂
    ...
    bool ProcessInput()
    {
        ...
        {
            if (NetWorkFrame.s_nReceiveCount < 0)
            {
                NetWorkFrame.s_nReceiveCount = 0;
            }
            NetWorkFrame.s_nReceiveCount += (int)(nSizeAfter - nSizeBefore);
        }
        return true;
    }
    ...
    void ProcessPacket()
    {
        ...
        while (m_bCanProcessPacket && (nProcessPacketCount--) > 0)
        {
            ...
            try
            {
                ...
                pPacket = m_PacketFactoryManager.GetPacketHandler(messageid);
                ...
                PacketDistributed realPacket = PacketDistributed.CreatePacket(messageid);
    ...
    public void SendPacket(PacketDistributed pPacket)
    {
        ...
        Debug.Log("SendPacket:" + pPacket.ToString());
        ...
        if (m_Socket.IsValid)
        {
            ...
            Int32 nlen = nValidbyteSize + NetWorkFrame.PACKET_HEADER_SIZE - 4;
            ...
            if (nSizeAfter > nSizeBefore)
            {
                if (NetWorkFrame.s_nSendCount < 0)
                {
                    NetWorkFrame.s_nSendCount = 0;
                }
                NetWorkFrame.s_nSendCount += (int)(nSizeAfter - nSizeBefore);
            }
        }
        else
        {
    ...
    public void ConnectThread()
    {
        ...
        while (true)
        {
            ...
            {
                Debug.LogWarning(m_strConnectResult);
            }
            m_Socket.close();
    ...
    {
        NetWorkFrame rMe = me as NetWorkFrame;
        rMe.ConnectThread();
    }
